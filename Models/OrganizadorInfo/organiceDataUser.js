...
async function organizeByAlphabetUsersSe(data) {
    var organizedData = {};
    data.forEach(username => {
        var firstLetter = username.trim()[0].toUpperCase(); // Obtener la primera letra y convertirla a mayúscula
        // Verificar si firstLetter es una letra
        if (/^[A-Z]$/.test(firstLetter)) {
            // Si no existe este medio lo crea
            if (!organizedData[firstLetter]) {
            ...
            organizedData[firstLetter].push(username);
        } else {
            // Si no existe este medio lo crea
            if (!organizedData['Others']) {
                organizedData['Others'] = [];
            }
            organizedData['Others'].push(username);
        }
    });
